...
module.exports = function mixin(cls) {
  return class ExchangeWs extends cls {
    ...
    wsHandleSubscribeData(data) {
      ...
      if (topic) {
        const name = topic[topic.length - 1]
        const handler = this['subscribeHandler' + name]
    ...
    static defineSubscriber(name, topicParser, eventHandler) {
      this.prototype['subscribeHandler' + name] = eventHandler
      this.prototype['subscribe' + name] = function(...args) {
        return Promise.resolve(topicParser.call(this, ...args)).then(res => {
          return this.subscribe(res[0], ...res[1], name)
        })
      }
      this.prototype['unsubscribe' + name] = function(...args) {
        return Promise.resolve(topicParser.call(this, ...args)).then(res => {
          return this.subscribe(res[0], ...res[2])
        })
      }
    }
  }
