...
class binanceWs extends ExchangeWs(binance) {
  ...
  wsHandleResponseData(msg) {
    if (msg.id || msg.error) {
      if (!msg.error) return { id: msg.id, data: msg }
      let err = new Error(msg.error['msg'])
      err.code = msg.error['code']
      // binance error has not request id.
      return { id: msg.id || this._lastRequestId(), error: err }
    }
  }

  wsHandleSubscribeTopic(data) {
    return this.getTopic(data.stream)
  }
...
binanceWs.defineSubscriber(
  ...
  async function topicParser(symbol) {
    ...
    return `${market.id.toLowerCase()}@trade`
  },
